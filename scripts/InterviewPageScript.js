...
document.addEventListener("DOMContentLoaded", function () {
  ...
  radioButtons.forEach((radio) => {
    radio.addEventListener("change", function () {
      ...
      if (questionContainer.classList.contains("tf-category")) {
        ...
        questionContainer.classList.contains("tf-explanation-category")
      ) {
        if (this.value === "true") {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const tfQuestions = document.querySelectorAll(
      ".tf-category, .tf-explanation-category"
    );
    tfQuestions.forEach((question) => {
      ...
      if (question.classList.contains("tf-explanation-category")) {
        const yesSelected = Array.from(radioButtons).find(
          (radio) => radio.value === "true" && radio.checked
        );
